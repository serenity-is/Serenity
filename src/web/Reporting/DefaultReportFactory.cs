    ...
    IHttpContextAccessor? httpContextAccessor = null) : IReportFactory
{
    ...
    public IReport Create(string reportKey, string? reportOptions, bool validatePermission)
    {
        ...
        var report = (IReport)ActivatorUtilities.CreateInstance(requestServices, reportInfo.Type);
    ...
    public void SetParams(IReport report, string reportOptions)
    {
        ...
        var trimmedOptions = reportOptions.TrimToNull();
        if (trimmedOptions != null)
            JSON.PopulateObject(report, trimmedOptions, JSON.Defaults.Strict);
    }
}