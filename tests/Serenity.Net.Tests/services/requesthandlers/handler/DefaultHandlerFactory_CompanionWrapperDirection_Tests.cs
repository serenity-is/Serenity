...
#pragma warning restore CS0649
    ...
    /// <summary>
    /// The documented contract of CompanionHandlerTypeAttribute: the wrapper adapts a handler
    /// implementing CompanionType to the interface the attribute is applied to. A wrapper that
    /// produces the companion interface instead is wired backwards.
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// DefaultHandlerFactory does Activator.CreateInstance(wrapperType, customHandler) where the
    /// custom handler implements CompanionType, so the wrapper must have such a constructor.
    /// </summary>
    [Theory]
    [MemberData(nameof(ProcessorInterfaces))]
