...
public class Int32FieldTests
{
    ...
    public void Constructor_SetsBasicProperties()
    {
        ...
        var field = new Int32Field(fields, "Test", "Db.Test", 100, FieldFlags.NotNull);
    ...
    {
        var field = new Int32Field(null, "Test");
    ...
    {
        var fields = new AllFieldsRow.RowFields();
        var field = Int32Field.Factory(fields, "Test", null, 0, FieldFlags.Default, null!, null!);
        Assert.IsType<Int32Field>(field);
        Assert.Equal("Test", field.Name);
    }
    ...
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
        field[row] = 5;
    ...
    public void GetFromReader_Null_SetsNull()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
        using var reader = new MockDbDataReader([new { AInt32 = (int?)null }]);
        reader.Read();
    ...
    public void GetFromReader_Value_SetsValue()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
        using var reader = new MockDbDataReader([new { AInt32 = 5 }]);
        reader.Read();
        field.GetFromReader(reader, 0, row);
        Assert.Equal(5, field[row]);
    }
    ...
    public void GetFromReader_NonIntValue_Converts()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
        using var reader = new MockDbDataReader([new { AInt32 = 5L }]);
        reader.Read();
        field.GetFromReader(reader, 0, row);
        Assert.Equal(5, field[row]);
    }
    ...
    {
        var row = NewRow();
        Assert.Throws<ArgumentNullException>(() => AllFieldsRow.Fields.AInt32.GetFromReader(null!, 0, row));
    }
    ...
    [InlineData("false", 0)]
    [InlineData("\"42\"", 42)]
    public void ValueFromJson_Newtonsoft_ParsesTokens(string json, int? expected)
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
    ...
    {
        var row = NewRow();
        Assert.Throws<ArgumentNullException>(() => AllFieldsRow.Fields.AInt32.ValueFromJson(null!, row, Newtonsoft.Json.JsonSerializer.CreateDefault()));
    }
    ...
    [InlineData("false", 0)]
    [InlineData("\"42\"", 42)]
    public void ValueFromJson_Stj_ParsesTokens(string json, int? expected)
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.AInt32;
    ...
    {
        var field = AllFieldsRow.Fields.AInt32;
        Assert.Equal(5, field.ConvertValue(new Newtonsoft.Json.Linq.JValue(5), CultureInfo.InvariantCulture));
    }
