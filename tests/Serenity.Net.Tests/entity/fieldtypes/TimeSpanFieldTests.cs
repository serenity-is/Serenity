...
public class TimeSpanFieldTests
{
    ...
    public void Constructor_SetsBasicProperties()
    {
        ...
        var field = new TimeSpanField(fields, "Test", "Db.Test", 100, FieldFlags.NotNull);

        Assert.Equal("Test", field.Name);
        Assert.Equal(FieldType.Time, field.Type);
    ...
    {
        var field = new TimeSpanField(null, "Test");
    ...
    public void Indexer_GetSet_Works()
    {
        ...
        field[row] = new TimeSpan(10, 30, 0);
    ...
    public void GetFromReader_Null_SetsNull()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        using var reader = new MockDbDataReader([new { ATimeSpan = (TimeSpan?)null }]);
        reader.Read();
    ...
    public void GetFromReader_Value_SetsValue()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        using var reader = new MockDbDataReader([new { ATimeSpan = (TimeSpan?)new TimeSpan(10, 30, 0) }]);
        reader.Read();
        field.GetFromReader(reader, 0, row);
        Assert.Equal(new TimeSpan(10, 30, 0), field[row]);
    }
    ...
    [Fact]
    public void GetFromReader_DateTimeValue_ConvertsToTimeOfDay()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        using var reader = new MockDbDataReader([new { ATimeSpan = (DateTime?)new DateTime(2024, 1, 15, 10, 30, 0) }]);
        reader.Read();
        field.GetFromReader(reader, 0, row);
        Assert.Equal(new TimeSpan(10, 30, 0), field[row]);
    }
    ...
    [InlineData("10:30:00", null, 1)]
    [InlineData(null, "10:30:00", -1)]
    [InlineData("10:30:00", "10:30:00", 0)]
    [InlineData("09:30:00", "10:30:00", -1)]
    [InlineData("10:30:00", "09:30:00", 1)]
    ...
    public void ValueToJson_Newtonsoft_WritesValue()
    {
        ...
        writer.Flush();
        Assert.Equal("\"10:30:00\"", sw.ToString());
    }
    ...
    public void ValueFromJson_Newtonsoft_String_Parses()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        using var reader = new Newtonsoft.Json.JsonTextReader(new System.IO.StringReader("\"10:30:00\""));
    ...
    [InlineData("\"2024-01-15T10:30:00Z\"", "10:30:00")]
    public void ValueFromJson_Stj_String_Parses(string json, string expected)
    {
        ...
        reader.Read();
        field.ValueFromJson(ref reader, row, new JsonSerializerOptions());
        Assert.Equal(TimeSpan.Parse(expected, CultureInfo.InvariantCulture), field[row]);
    }
    ...
    public void Copy_CopiesValue()
    {
        ...
        var target = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        field[source] = new TimeSpan(10, 30, 0);
        field.Copy(source, target);
        Assert.Equal(new TimeSpan(10, 30, 0), field[target]);
    }
    ...
    public void AsObject_SetsValue()
    {
        var row = NewRow();
        var field = AllFieldsRow.Fields.ATimeSpan;
        field.AsObject(row, new TimeSpan(10, 30, 0));
        Assert.Equal(new TimeSpan(10, 30, 0), field[row]);
    ...
    [Fact]
    public void ConvertValue_TimeSpan_PassesThrough()
    {
        var field = AllFieldsRow.Fields.ATimeSpan;
        var ts = new TimeSpan(10, 30, 0);
        Assert.Equal(ts, field.ConvertValue(ts, CultureInfo.InvariantCulture));
    }
    ...
    {
        var field = AllFieldsRow.Fields.ATimeSpan;
        Assert.Equal(new TimeSpan(10, 30, 0), field.ConvertValue(new Newtonsoft.Json.Linq.JValue(new TimeSpan(10, 30, 0)), CultureInfo.InvariantCulture));
    }
}
