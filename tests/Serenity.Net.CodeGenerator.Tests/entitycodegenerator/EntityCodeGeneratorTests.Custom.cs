...
public partial class EntityCodeGeneratorTests
{
    ...
    {
        var generator = Create(out var fs, out var templates, out var writer, out _, out var config);
    ...
    {
        var generator = Create(out var fs, out _, out var writer, out _, out var config);
        config.GenerateCustom = true;
        config.CustomGenerate = new Dictionary<string, string>()
        {
            ["Row"] = null!,
