...
Primitive branch values (string, number, boolean) are rendered through a `Text`
node, so `Show` still has an anchor for the branch lifecycle and for the
subscription to `when`.
