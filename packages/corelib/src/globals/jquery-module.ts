...
class $I extends Array<HTMLElement> {
    ...
    map(fn: any): any[] {
        return this.map((el: HTMLElement, i: number) => fn.call(el, i, el));
    }
    ...
    parent() {
        return $S([...Array.from(new Set(this.map((el: any) => el.parentNode as HTMLElement)))]);
    }
    ...
    filter(a: any): any {
        if (typeof a === 'string') {
            return $S(this.filter((el: any) => el.matches(a)));
...
$S.ajaxSetup = function (options: any) {
    this.ajaxOptions = Object.assign({ headers: {} }, options);
};

$S.ajax = function (params: any) {
...
$S.debounce = function (callback: any, wait: any) {
    let timeout: number;
...
$S.throttle = function (callback: any, limit: number) {
