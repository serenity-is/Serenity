...
namespace Serenity.Data
{
    public static class Config
    {
        ...
        public static object Get(Type settingType, bool cached = true)
        {
            ...
            if (typeof(ISiteSetting).IsAssignableFrom(settingType))
            {
                if (cached)
                    ...
                    return Json.Deserialize(IoC.Resolve<ISiteStateService>().Load(key).TrimToNull() ?? "{}", settingType);
            }
            else
            {
                if (cached)
                    ...
                    return Json.Deserialize(IoC.Resolve<ILocalUserStateService>().Load(key).TrimToNull() ?? "{}", settingType);
            }
        }
        ...
        public static TSettings Get<TSettings>(bool cached = true) where TSettings : class, ISettings, new()
        {
            ...
            if (typeof(ISiteSetting).IsAssignableFrom(typeof(TSettings)))
            {
                if (cached)
                    ...
                    return Json.Deserialize<TSettings>(IoC.Resolve<ISiteStateService>().Load(key).TrimToNull() ?? "{}");
            }
            else
            {
                if (cached)
                    ...
                    return Json.Deserialize<TSettings>(IoC.Resolve<ILocalUserStateService>().Load(key).TrimToNull() ?? "{}");
            }
        }
        ...
        public static void Set(object state)
        {
            ...
            string stateString = Json.SerializeIndented(state);
