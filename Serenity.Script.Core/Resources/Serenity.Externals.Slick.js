...
$.widget("ui.slickPager", {
    options: {
        ...
        onRpChange: null,
        pageStatusText: 'Toplam {total} kayıttan {from} ile {to} arasındakiler gösteriliyor',
        noRowStatusText: 'Kayıt yok',
        loadingStatusText: 'Veri alınıyor, lütfen bekleyiniz...'
    },
                ...
                '<div class="slick-pg-in">' + 
                    '<div class="slick-pg-grp">' +
                     ...
                     '</div><div class="slick-pg-sep"></div><div class="slick-pg-grp">' + 
                         '<span class="slick-pg-control">Sayfa <input class="slick-pg-current" type="text" size="4" value="1" /> / ' + 
    ...
    _updatePager: function () { 
        ...
        if (info.loading) {
            stat = this.options.loadingStatusText;
        }
        else if (info.error) {
            stat = info.error;
        }
        else if (info.totalRows > 0) {
            stat = this.options.pageStatusText;
            stat = stat.replace(/{from}/, r1);
            stat = stat.replace(/{to}/, r2);
            stat = stat.replace(/{total}/, info.totalRows);
        }
        else
            stat = this.options.noRowStatusText;
