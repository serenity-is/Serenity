(function() {
	...
	$Serenity_Test_EmailEditorTests.$createValidatedForm = function() {
		...
		form.validate(Q$Externals.validateOptions(Q$Externals.validateOptions({})));
		return form;
	};
	...
	ss.initClass($Serenity_Test_QDialogTests, $asm, {
		runTests: function() {
			...
			test('ConfirmDialogCanOpenClose', ss.mkdel(this, function() {
				...
				var yesButton = $(".ui-dialog-buttonset button:contains('" + Texts$Dialogs.YesButton.get() + "')");
				ok(yesButton.length === 1, 'Check that dialog has Yes button');
				var noButton = $(".ui-dialog-buttonset button:contains('" + Texts$Dialogs.NoButton.get() + "')");
			...
			test('ConfirmDialogYesClickCallsSuccess', ss.mkdel(this, function() {
				...
				$(".ui-dialog-buttonset button:contains('" + Texts$Dialogs.YesButton.get() + "')").click();
			...
			test('ConfirmDialogNoClickDoesntCallSuccess', ss.mkdel(this, function() {
				...
				$(".ui-dialog-buttonset button:contains('" + Texts$Dialogs.NoButton.get() + "')").click();
	...
	ss.initClass($Serenity_Test_QExternalsTests, $asm, {
		runTests: function() {
			test('TurkishLocaleCompareWorksProperly', ss.mkdel(this, function() {
				deepEqual(Q$Externals.turkishLocaleCompare('a', 'b'), -1);
				deepEqual(Q$Externals.turkishLocaleCompare('b', 'a'), 1);
				deepEqual(Q$Externals.turkishLocaleCompare('b', 'b'), 0);
				deepEqual(Q$Externals.turkishLocaleCompare('i', 'ı'), 1);
				deepEqual(Q$Externals.turkishLocaleCompare('ı', 'i'), -1);
				deepEqual(Q$Externals.turkishLocaleCompare('İSTANBUL', 'ıSpaRTA'), 1);
				deepEqual(Q$Externals.turkishLocaleCompare('ıSpaRTA', 'İSTANBUL'), -1);
				deepEqual(Q$Externals.turkishLocaleCompare('0', 'TEXT'), -1);
				deepEqual(Q$Externals.turkishLocaleCompare('TEXT', '5'), 1);
				deepEqual(Q$Externals.turkishLocaleCompare('123ABC', '123AB'), 1);
				deepEqual(Q$Externals.turkishLocaleCompare('', null), 0);
				deepEqual(Q$Externals.turkishLocaleCompare('_', null), 1);
				deepEqual(Q$Externals.turkishLocaleCompare(null, 'X'), -1);
			}));
		}
	});
	...
	ss.initClass($Serenity_Test_QMethodTests, $asm, {
		runTests: function() {
			test('ToJSONWorks', ss.mkdel(this, function() {
				...
				deepEqual($.toJSON(date), '"' + Q$Externals.formatISODateTimeUTC(date) + '"', 'Date/Time');
				...
				o.date = Q$Externals.formatISODateTimeUTC(date);
